...
# The IDs of all ARM64 cisagov/egress-assess-packer AMIs
...
#   filter {
#     name   = "name"
#     values = ["egress-assess-hvm-*-arm64-ebs"]
#   }
#
#   filter {
...
#   }
#
#   account_name_regex   = local.ami_share_account_name_regex
#   ami_id               = each.value
#   extraorg_account_ids = var.extraorg_account_ids
# }

# The IDs of all x86-64 cisagov/egress-assess-packer AMIs
data "aws_ami_ids" "historical_amis_x86_64" {
  ...
  filter {
    name   = "name"
    values = ["egress-assess-hvm-*-x86_64-ebs"]
  }
